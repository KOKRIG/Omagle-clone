/*
  # Olyx.site Database Schema
  
  1. Tables Created
    - `profiles` - User profile information including gender, country, plan, ban status
    - `match_queue` - Active matchmaking queue for finding matches
    - `active_matches` - Current chat sessions with WebRTC signaling data
    - `ice_candidates` - ICE candidates for WebRTC peer connections
    - `reports` - User reports for inappropriate behavior
  
  2. Security
    - Enable RLS on all tables
    - Add policies for user access control
    - Profiles: Users can manage their own profile
    - Match queue: Users can join/leave queue, view for matching
    - Active matches: Users can manage their own matches
    - ICE candidates: Users can send/receive for their matches
    - Reports: Users can create reports (read restricted to admin)
  
  3. Functions
    - `increment_report_count` - Increment report count and apply ban if threshold reached
    - `is_user_banned` - Check if user is currently banned
    - `check_billing_status` - Check and update billing status
  
  4. Realtime
    - Enable realtime subscriptions on match_queue, active_matches, ice_candidates, profiles
...
-- Allow reading other profiles for matchmaking (limited fields)
